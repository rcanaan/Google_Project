...
def calc_score(user_query: str, data: str):
    ...
    index = 0
    sub = 0
    if abs(len(user_query) - len(data)) > 1:  # case of more than 1 change at the letters?
        return False, sub
    # adding or subtracting a word
        ...
        if len(user_query) > len(data):
            word1 = user_query
            word2 = data
        else:  # the query_word is shorter than the data word
            word1 = data
            word2 = user_query
        for letter in word1:
            if len(word2) == index and mistake_flag:  # if we finished going through the whole word2 and there is a mistake
                return False, sub
                ...
                mistake_flag = True
                save_index = index
            else:  # 2 consecutive letters at word that the same as
                index += 1
    else:
